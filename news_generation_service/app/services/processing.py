...
def generate_ai_summaries():
    ...
    Key design: the DB session is CLOSED before calling the Gemini API.
    This ensures zero PostgreSQL locks are held during any external API call,
    so news_service can always serve reads in parallel without contention.
    """
    if not GEMINI_API_KEY or GEMINI_API_KEY == "your_gemini_api_key_here":
        print("Gemini API key missing. Skipping AI summarization.", flush=True)
        return

    # --- Phase 1: Collect all work, then CLOSE the DB session ---
    # We fetch everything we need upfront and immediately release the connection.
    pending_work = []  # list of (cluster_id, [article_dicts], context_text)
    db = SessionLocal()
    try:
        cutoff_48h = datetime.utcnow() - timedelta(hours=48)
        # Cap at 15 clusters per run. Each cluster needs a Gemini API call + 5s sleep,
        # so 15 clusters ≈ ~2-3 minutes max, keeping the total pipeline runtime short.
        ...
        for cluster_id, articles in cluster_map.items():
            all_cluster_articles = db.query(models.Article).filter(
            ...
            # Store only primitive data — no ORM objects outside the session
            article_ids = [a.id for a in articles]
            pending_work.append((cluster_id, article_ids, context_text))

    except Exception as e:
        print(f"Error collecting work for AI summarization: {e}", flush=True)
        return
    finally:
    ...
    api_keys = [k for k in [primary_key, secondary_key, tertiary_key] if k]
    current_key_idx = 0
    rate_limit_hits = 0
    ...
    # --- Phase 2: Call Gemini with NO open DB session ---
    for cluster_id, article_ids, context_text in pending_work:
        if api_keys:
        ...
        print(f"Generating Gemini SEO summary and analysis for Cluster ID {cluster_id}...", flush=True)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if data and ('ai_summary' in data or 'full_analysis' in data):
                    ...
                    print(f"Attempt {attempt + 1}: Data missing required keys. Raw: {text[:200]}...", flush=True)

            except Exception as e:
                error_msg = str(e)
                print(f"Gemini Attempt {attempt + 1} failed for cluster {cluster_id}: {error_msg[:200]}", flush=True)

                if "blocked" in error_msg.lower() or "response.parts quick accessor" in error_msg:
                    result_data = {"skip": True}
                    break

                if "429" in error_msg or "RESOURCE_EXHAUSTED" in error_msg:
                    rate_limit_hits += 1
                    if rate_limit_hits > 3 and model_name != fallback_model_name:
                        print(f"Too many rate limits, falling back to {fallback_model_name}", flush=True)
        ...
        if result_data and not result_data.get("skip"):
            db_write = SessionLocal()
            try:
                primary_article = db_write.query(models.Article).filter(
                    models.Article.id == article_ids[0]
                ).first()

                if primary_article:
                    primary_article.ai_summary = result_data.get('ai_summary')
                    primary_article.full_analysis = result_data.get('full_analysis')
                    raw_slug = result_data.get('slug') or primary_article.slug or "news"
                    clean_slug = re.sub(r'[^a-zA-Z0-9]+', '-', str(raw_slug).lower()).strip('-')
                    primary_article.slug = f"{clean_slug}-{uuid.uuid4().hex[:8]}"
                    primary_article.meta_title = result_data.get('meta_title')
                    primary_article.meta_description = result_data.get('meta_description')
                    primary_article.is_verified = bool(result_data.get('is_genuine', False))
                    primary_article.published_at = datetime.utcnow()
                    if result_data.get('category'):
                        primary_article.category = result_data.get('category')

                    # Mark secondary articles as non-verified (they're related sources)
                    for other_id in article_ids[1:]:
                        other = db_write.query(models.Article).filter(models.Article.id == other_id).first()
                        if other:
                            other.is_verified = False

                    db_write.commit()
                    print(f"Successfully generated summary for cluster {cluster_id}", flush=True)
            except Exception as e:
                db_write.rollback()
                print(f"Failed to write result for cluster {cluster_id}: {e}", flush=True)
            finally:
                db_write.close()  # <-- Immediately released after write
        elif result_data and result_data.get("skip"):
            db_write = SessionLocal()
            try:
                a = db_write.query(models.Article).filter(models.Article.id == article_ids[0]).first()
                if a:
                    a.ai_summary = "[GENERATION_SKIPPED_SAFETY]"
                    db_write.commit()
            except Exception:
                db_write.rollback()
            finally:
            ...
            try:
                a = db_write.query(models.Article).filter(models.Article.id == article_ids[0]).first()
                if a:
                    a.ai_summary = "[GENERATION_FAILED]"
                    db_write.commit()
            except Exception:
                db_write.rollback()
            finally:
